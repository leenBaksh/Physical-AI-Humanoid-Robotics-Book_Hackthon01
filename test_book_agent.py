#!/usr/bin/env python3
"""
Test to see if BookAgent can be initialized properly
"""
...
print("Testing BookAgent initialization...")
...
try:
    ...
    async def test_agent():
        ...
        print(f"Agent response: {result['answer'][:100]}...")
        print(f"Retrieved chunks: {len(result.get('retrieved_chunks_used', []))}")

    asyncio.run(test_agent())
    print("SUCCESS: BookAgent is working correctly!")

except Exception as e:
    print(f"ERROR during BookAgent test: {e}")
    import traceback
    traceback.print_exc()